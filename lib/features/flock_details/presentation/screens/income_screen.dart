...
import 'package:finalproject/features/flock_details/presentation/widgets/items/income_item.dart';
...
class IncomeCategoryScreen extends StatefulWidget {
  const IncomeCategoryScreen({super.key, required this.flock});
...
class _IncomeCategoryScreenState extends State<IncomeCategoryScreen> {
  ...
  void initState() {
    BlocProvider.of<RecordsCubit>(context)
        .getRecord('income/${widget.flock.sId}');
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: Column(children: [
          ...
          const Text('Income', style: MyFonts.textStyle24),
            ...
            builder: (context, state) {
              if (state is RecordsSuccess) {
                ...
                if (myRecords.isEmpty) {
                  return const NoData(title: 'Incomes');
                } else {
                  var cubitSearching =
                      BlocProvider.of<SearchCubit>(context).searchingIncome;
                  return Expanded(
                    child: Column(children: [
                      ...
                      BlocBuilder<SearchCubit, SearchState>(
                        builder: (context, state2) {
                          if (state2 is SearchingState) {
                            myRecords = state2.recordItems;
                          } else {
                          ...
                          return Expanded(
                            child: ListView.builder(
                              ...
                              itemBuilder: (ctx, idx) => IncomeItem(
                                  flockModel: widget.flock,
                                  incomeModel: myRecords[idx] as IncomeModel),
                            ),
                          );
                        },
                      )
        ...
        floatingActionButton: FloatingAddButton(
          navigate: () async {
            final res = await Navigator.of(context)
                .pushNamed(addIncome, arguments: {'flock_model': widget.flock});
            if (res == true) {
              BlocProvider.of<RecordsCubit>(context)
                  .getRecord('income/${widget.flock.sId}');
            }
